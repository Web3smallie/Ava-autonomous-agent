...
// Connect to server for live state updates
let serverApp = null;
try { serverApp = require("../api/server"); } catch(e) {}
...
async function getWalletBalances() {
  ...
  const [usdtBal, wethBal] = await Promise.all([
...
async function runCycle() {
  ...
  try {
    ...
    console.log(`💰 Balances: ${balances.usdt} USDT | ${balances.weth.toFixed(6)} WETH`);
    ...
    if (balances.usdt < 2 && balances.weth > 0.0001) {
      ...
      const txHash = await executeSwap(sellDecision);
      if (txHash && serverApp && serverApp.updateState) {
        serverApp.updateState(sellDecision, {
          action: "SELL",
          token: "ETH-USDT",
          tx: txHash,
    ...
    // If no funds at all
    if (balances.usdt < 1 && balances.weth < 0.0001) {
      console.log("💀 AVA is out of funds — entering survival mode");
      return;
    }
    ...
    // Update server state with latest decision
    if (serverApp && serverApp.updateState) {
      serverApp.updateState(decision, null);
    }
    ...
    if (decision.confidence >= 0.65 && decision.action !== "HOLD") {
      ...
      if (txHash) {
        ...
        // Update server with trade
        if (serverApp && serverApp.updateState) {
          serverApp.updateState(null, {
